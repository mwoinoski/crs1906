...
def test_add_user_ok():
    ...
    r = requests.post(url, auth=creds, headers=http_headers, data=json_data)
...
def test_update_user_ok():
    ...
    json_data = json.dumps(expected_result)

    r = requests.put(url, auth=creds, headers=http_headers, data=json_data)
