...
class BaseDao:
    ...
    def __init__(self, entity_class, id_attr):
        ...
        # TODO: search entity_class.__dict__ for attr where
        # issubclass(attr.__class__, Column) and attr.primary_key

    def get(self, id_value, db_session):
        logger.debug("%s: get %s %s = %s", func_name(self),
                     self._entity_class.__class__.__name__, self._id_attr, id_value)
        entity = db_session.query(self._entity_class)\
                           .filter(getattr(self._entity_class, self._id_attr) == id_value)\
                           .first()
        # Example filter query: .filter(Person.email == id_value)
        if not entity:
            raise PersistenceError("Can't delete {} with id {}: not in database"
                                   .format(self._entity_class, id_value))
