...
Note that several tests initially fail because some Person methods are not
...
class PersonTestCase(unittest.TestCase):
    ...
    def test_full_name_last_only(self):
        last_name = "Coltrane"
        person = Person(None, None, last_name)

        full_name = person.full_name()

        self.assertEqual(last_name, full_name)

    # fails because Person.full_name() isn't fully implemented
    def test_full_name_first_only(self):
        first_name = "Miles"
        person = Person(first_name, None, None)

        full_name = person.full_name()

        self.assertEqual(first_name, full_name)

    # fails because Person.full_name() isn't fully implemented
    def test_full_name_middle_only(self):
        middle_name = "Quincy"
        person = Person(None, middle_name, None)

        full_name = person.full_name()

        self.assertEqual(middle_name, full_name)
